...
class BridgeDefense:
    ...
    def _get_ip_address(self):
            ...
            try:
                ...
                addr_info = socket.getaddrinfo(self._hostname, self._port1, socket.SOCK_DGRAM, proto=socket.IPPROTO_UDP)
        ...
        while remaining_attempts > 0:
            try:
                ...
                with socket.socket(address_family, socket.SOCK_DGRAM) as client_socket:
                    ...
                    client_socket.sendto(jsonRequest.encode(), (ip_address, self._port1+serverNum))
                    ...
                    if(dictResponse["type"] == "gameover" and dictResponse["status"] == 1):
                        print("JOGO ENCERRADO: " + dictResponse["description"])
                        return None
                    elif(dictResponse["type"] == "gameover" and dictResponse["status"] == 0):
                ...
                if remaining_attempts == 0:
                    print("Não foi possíel se conectar com o servidor. Verifique sua conexão, o hostname e a porta.")
                    return None
                else:
                    print(f"Ocorreu um timeout ao tentar conexão com o servidor {serverNum}. Tentando novamente...")
    ...
    def _cannonPlacementRequest(self):
        # Método chamado somente ao iniciar um novo jogo para saber a posição dos canhões
        # no fim deve escrever na variável "__cannons" que será acessada por outros métodos
        # CUIDADO ao testar, porque solicitar canhões inicia um novo jogo. 
        # Então sempre deve ser chamado o método "__gameTerminationRequest()" para encerrar o jogo.
        # Obs.: armazenar o turno atual na variável "__currentTurn" para ser usado em outros métodos
        return 
    ...
    def _gameTerminationRequest(self):
        # Esse método não precisa retornar nada, só encerrar o jogo
        # Mensagens de "game over" são tratadas a partir do método "__serverCommunication()"
        return
    ...
    def playGame(self):
        ...
        if (self._authenticationRequest() == False):
            print("Para continuar é preciso autenticar em todos os rios. Tente novamente.")
            exit(0)

        # ETAPA2: Inicia o jogo e recebe a posição dos canhões (a implementar)
        # desenvolver método "_cannonPlacementRequest()"
...
if __name__ == "__main__":
    ...
    if num_args != 4:
      print("Parâmetros incorretos. Como usar: python client.py <hostname> <port 1> <GAS>")
      sys.exit(1)
